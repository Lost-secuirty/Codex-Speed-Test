...
// =====================================================================
// The audit's own load-bearing logic (ADR-0007). checkDeviationSection
// enforces Working Agreement #10's PR-body section; historyLine/hasHead
...
describe('checkDeviationSection', () => {
  ...
  it('treats a section ending at end-of-body correctly', () => {
    expect(checkDeviationSection('## Deviations from plan\nSwapped the tween seam.')).toBeNull();
...
describe('learningsDistillDue', () => {
  const doc = (lines: number) => Array.from({ length: lines }, (_, i) => `line ${i}`).join('\n');
...
describe('CHECK_IDS', () => {
  it('is the canonical 14-check list with unique ids', () => {
    ...
    // this repo's addition vs the demo-math blueprint (ADR-0002)
    expect(CHECK_IDS).toContain('typecheck-fail');
  });
