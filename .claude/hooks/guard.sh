...
case "$file" in
  ...
  */__screenshots__/* | __screenshots__/*)
    echo "Refusing to hand-edit visual baselines — update only via 'npm run test:visual:update' (ADR-0006)." >&2
    exit 2
    ;;
  */PERSONAL_JOURNAL* | PERSONAL_JOURNAL*)
    echo "Refusing to touch PERSONAL_JOURNAL* — personal-tier file (Drive only)." >&2
    exit 2
    ;;
  */private/* | private/*)
