...
// no-overclaim honesty rule from ADR-0023: while STATUS.md records `agent_interop_phase: A`,
// the card must NOT advertise a live, callable endpoint. Pure Node stdlib — no new deps.
//
// `validateAgentSurface` and `parsePhase` are exported so scripts/gate-canary.mjs (canaryAgentCard)
// can prove the gate bites (a broken/overclaiming card must fail). `node scripts/agent-card-validate.mjs`
...
export function validateAgentSurface({ card, tools, phase }) {
  ...
  // --- honesty: no live endpoint while phase A (ADR-0023) ---
  ...
  if (phase === 'A') {
    ...
    req(
      ...
      `phase A: x-lifecycle.interopPhase (${lc?.interopPhase}) must match STATUS.md (${phase})`,
    );
  }
