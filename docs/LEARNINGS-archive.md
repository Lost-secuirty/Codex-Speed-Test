...
Older and superseded entries moved out of the active
[`LEARNINGS.md`](LEARNINGS.md) retrieval path on **2026-06-13** (the
`learnings-distill-due` distillation pass, Working Agreement #11). Entries keep
their original dates and wording — nothing is deleted, this is the historical
record. The lessons that still bite are distilled at the bottom of the active
`LEARNINGS.md` and the signature one promoted to `GOLDEN_RULES.md` #18.
